...
from rdtl.validator import RDTLValidator, validate_template


def test_simple_custom_block():
...
def test_custom_block_with_args():
    ...
    for child in ast.children:
        if isinstance(child, ast_nodes.GenericBlockTag):
            assert child.tag_name == "myblock"
            assert "arg1" in child.raw_args
            break
    else:
        pytest.fail("GenericBlockTag not found in AST")
...
def test_multiple_different_custom_blocks():
...
def test_tag_without_end_is_simple_tag():
    ...
    assert is_valid, f"Should accept as simple tag: {errors}"


def test_mismatched_custom_block_fails():
    ...
    assert not is_valid, "Should reject mismatched tags"


def test_formatter_preserves_custom_blocks():
...
{% endmyblock %}"""
    ...
    assert "{% endmyblock %}" in formatted
    assert "<p>Content</p>" in formatted


def test_custom_block_proper_nesting():
    ...
    assert not is_valid, "Should reject improper nesting"
    # Check for proper nesting error (either "closes before" or "Expected")
    assert any("closes before" in str(e) or "Expected" in str(e) for e in errors)


def test_custom_block_with_for_loop():
...
def test_case_insensitive_end_tag():
    ...
    assert is_valid, f"Should be valid (case insensitive). Errors: {errors}"


# Discovery mechanism tests


def test_discovery_finds_end_tags():
    ...
    assert "myblock" in discovered
    assert "another" in discovered


def test_discovery_merges_with_known_tags():
    ...
    assert "if" in validator.ALLOWED_BLOCKS
    assert "customblock" in validator.ALLOWED_BLOCKS


def test_no_duplicate_discovery():
